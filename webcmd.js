...
const searches = {
  ...
  rr: [
    "https://reading-room.csail.mit.edu/cgi-bin/user/search.cgi",
    "search",
...
function runcmd(cmd) {
  ...
  let fn;
  if (searches[cmd] != undefined) fn = search;
  else if (shortcuts[cmd] != undefined) fn = shortcut;
  else {
...
function toarray(args) {
  let a = new Array();
  for (let i = 0; i < args.length; i++) a[i] = args[i];
  return a;
}

function cgiurl(base, params) {
  let url = base + "?";
  for (let k in params) url += k + "=" + escape(params[k]) + "&";
  return url;
}
